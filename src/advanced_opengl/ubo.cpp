...
int main()
{
    ...
    // ------------------------------------
    Shader shader(
            "shaders/ubo/cube_vertx.glsl",
            "shaders/ubo/cube_frag.glsl");
    Shader skyboxshader(
            "shaders/ubo/skybox_vertx.glsl",
            "shaders/ubo/skybox_frag.glsl");
    ...
    // ---------------------------------
    // get block indices
    unsigned int uniformBlockIndexshader = glGetUniformBlockIndex(shader.getProgram(), "Matrices");

    //link block to binding point
    glUniformBlockBinding(shader.getProgram(), uniformBlockIndexshader, 0);
    ...
    // store the projection matrix in uniform block
    ...
    while (!glfwWindowShouldClose(app.window))
    {
        ...
        //set the view matrix in uniform block
        ...
        // set skyboxshader view matrix in uniform block
        shader.setMat4("view", view);
        glBindBuffer(GL_UNIFORM_BUFFER, uboMatrices);
        glBufferSubData(GL_UNIFORM_BUFFER, sizeof(glm::mat4), sizeof(glm::mat4), glm::value_ptr(view));
        glBindBuffer(GL_UNIFORM_BUFFER, 0);
